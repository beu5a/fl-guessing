...
def evaluate_cifar10(server_model_weights, central_test_dataset):
  keras_model = get_cifar10_cnn2()
  keras_model.compile(
      loss=tf.keras.losses.SparseCategoricalCrossentropy(),
      metrics=[tf.keras.metrics.SparseCategoricalAccuracy()]  
  )
  #tf.nest.map_structure(lambda x,y: x.assign(y), keras_model.trainable_variables , server_model_weights)
  keras_model.set_weights(server_model_weights)
  return keras_model.evaluate(central_test_dataset)
