...
# Computes ||a_i||_1 for every client


def get_client_coeff_norms(budgets, num_gradient_guesses):
    """
    This function computes the L1 norm of coefficients for each client based on 
    momentum factor, number of steps and number of guesses.

    For tau_i local steps and g_i guesses, the formula is:
    { tau_i - alpha^(g_i + 1) x (1 - alpha^{tau_i}) / (1 - alpha) } / (1 - alpha)

    Parameters: 
        budgets [int]: List of client budgets
        num_gradient_guesses [int]: List of guesses done by each client

    Returns:
        [float] of L1 norm of coefficients 
    """
    alpha = 0.9  # momentum factor
    coeff_norms = []
    for i in range(len(budgets)):
        tau_i = budgets[i]
        g_i = num_gradient_guesses[i]
        tmp = tau_i - (pow(alpha, g_i + 1) *
                       (1 - pow(alpha, tau_i))/(1 - alpha))
        coeff_norms.append(tmp/(1 - alpha))
...
def get_client_agg_weights(budgets, client_num_samples, batch_size):
    ...
    agg_weights = [
        float(x)/float(total_seen_training_samples) for x in seen_training_samples]

    return agg_weights

# Return equal weights for all clients
# To be utilised in the case where budget << n_i/B


def get_client_agg_weights2(budgets, client_num_samples, batch_size):
    num_clients = len(budgets)
    w_i = 1.0/num_clients
    return [w_i]*len(budgets)

# Computes tau_eff = sum p_i x tau_i


def get_server_scaling_factor(agg_weights, budgets, client_coeff_norms):
    tau_eff = 0.0
    for i in range(len(budgets)):
        tau_eff += agg_weights[i] * budgets[i]
    return 6.0*tau_eff

# Computes tau_eff = sum p_i x ||a_i||_1


def get_server_scaling_factor2(agg_weights, budgets, client_coeff_norms):
    tau_eff = 0.0
    for i in range(len(agg_weights)):
        tau_eff += agg_weights[i] * client_coeff_norms[i]
    return tau_eff
...
def get_num_guesses(n_guesses, num_clients, budgets, lower_bound, upper_bound):
    ...
    if(n_guesses == 'max'):
        ...
        num_gradient_guesses = [
            abs(int((gap - abs(mid_u - b))/gap * 10)) for b in budgets]
    elif(n_guesses[0] == 'g'):
    ...
    # percentage guessing --> p20 would mean 20% of budget
        ...
        percentage_g = float(n_guesses[1:])/100.0
        num_gradient_guesses = [int(floor(percentage_g * b)) for b in budgets]
        budgets = [budgets[i] - num_gradient_guesses[i]
                   for i in range(len(budgets))]
...
def run_fl(
    ...
    with tf.summary.create_file_writer(log_dir).as_default():
        while(True):
            ...
            client_coeff_norms = get_client_coeff_norms(
                budgets, num_gradient_guesses)
            client_agg_weights = get_client_agg_weights(
                budgets, client_num_samples, hparams['batch_size'])
            server_scaling_factor = get_server_scaling_factor2(
                client_agg_weights, budgets, client_coeff_norms)
